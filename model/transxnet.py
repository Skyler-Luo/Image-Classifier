...
class TransXNet(nn.Module):
    ...
    '''
    init for semantic segmentation tasks
...
def transxnet_t(pretrained=False, pretrained_cfg=None, **kwargs):
    ...
    if pretrained:
        ckpt_url = 'https://github.com/LMMMEng/TransXNet/releases/download/v1.0/transx-t.pth.tar'
...
def transxnet_s(pretrained=False, pretrained_cfg=None, **kwargs):
    ...
    model.default_cfg = _cfg(crop_pct=0.9)
    if pretrained:
        ckpt_url = 'https://github.com/LMMMEng/TransXNet/releases/download/v1.0/transx-s.pth.tar'
        checkpoint = torch.hub.load_state_dict_from_url(ckpt_url, map_location='cpu')
        model.load_state_dict(checkpoint, strict=False)
    return model
...
def transxnet_b(pretrained=False, pretrained_cfg=None, **kwargs):
    ...
    if pretrained:
        ckpt_url = 'https://github.com/LMMMEng/TransXNet/releases/download/v1.0/transx-b.pth.tar'
        checkpoint = torch.hub.load_state_dict_from_url(ckpt_url, map_location='cpu')
        model.load_state_dict(checkpoint, strict=False)
    return model
...
if __name__ == '__main__':
    ...
    model = transxnet_t(pretrained=False)
    model.eval()
    out = model(inputs)
    print('out shape:{}'.format(out.size()))
    
    # 测试fork_feat模式（用于语义分割）
    model_seg = transxnet_t(pretrained=False, fork_feat=True)
    model_seg.eval()
    feas = model_seg(inputs)
