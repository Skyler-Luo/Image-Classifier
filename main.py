...
from utils.utils import (
    ...
    setting_optimizer, show_config, update_opt
)
...
def parse_opt():
    ...
    return opt, model, ema, train_dataset, test_dataset, optimizer, scaler, lr_scheduler, loss, DEVICE, CLASS_NUM, (
        ckpt['epoch'] if opt.resume else 0), (ckpt['best_metric'] if opt.resume else None)


if __name__ == '__main__':
    opt, model, ema, train_dataset, test_dataset, optimizer, scaler, lr_scheduler, loss, DEVICE, CLASS_NUM, begin_epoch, best_metric = parse_opt()
    ...
    writer = SummaryWriter(log_dir=os.path.join(opt.save_path, 'tensorboard'))
    ...
    for epoch in range(begin_epoch, opt.epoch):
        ...
        begin = time.time()
        if opt.kd:
            metric = fitting_distill(teacher_model, model, ema, loss, kd_loss, optimizer, train_dataset, test_dataset, CLASS_NUM, DEVICE, scaler, '{}/{}'.format(epoch + 1,opt.epoch), opt)
        else:
            metric = fitting(model, ema, loss, optimizer, train_dataset, test_dataset, CLASS_NUM, DEVICE, scaler,'{}/{}'.format(epoch + 1, opt.epoch), opt)
