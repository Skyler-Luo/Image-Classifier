...
from utils.utils import cam_visual, dict_to_PrettyTable, model_fuse, predict_single_image, select_device
...
def parse_opt():
    ...
    if opt.half and opt.cam_visual:
        raise ValueError('cam visual only supported cpu. please set device=cpu.')
    if (opt.device != 'cpu') and opt.cam_visual:
        raise ValueError('cam visual only supported FP32.')
