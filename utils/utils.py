...
from pytorch_grad_cam import (
    AblationCAM, EigenCAM, FullGrad, GradCAM, GradCAMPlusPlus,
    HiResCAM, ScoreCAM, XGradCAM
)
...
def mixup_data(x, opt, alpha=1.0):
    ...
    if opt.mixup == 'mixup':
        ...
        raise ValueError('Unsupported MixUp Methods.')
    return mixed_x
...
class TrainMetric:
    ...
    def _calc_f1_from_cm(self, cm):
        """Calculate macro-average F1 score from confusion matrix"""
        precision_per_class = np.diag(cm) / (cm.sum(axis=0) + 1e-7)
        recall_per_class = np.diag(cm) / (cm.sum(axis=1) + 1e-7)
    ...
    def get(self):
        ...
        result['train_f1'] = self._calc_f1_from_cm(self.train_cm)
        result['test_f1'] = self._calc_f1_from_cm(self.test_cm)

        if 'train_kd_loss' in result:
            cols_name = ['train_loss', 'train_kd_loss', 'train_acc', 'train_mean_acc', 'train_f1', 'test_loss', 'test_acc', 'test_mean_acc', 'test_f1']
        else:
            cols_name = ['train_loss', 'train_acc', 'train_mean_acc', 'train_f1', 'test_loss', 'test_acc', 'test_mean_acc', 'test_f1']
...
class TestMetric:
    def __init__(self, y_true, y_pred, class_num):
        ...
        self.metrics = ['PPV', 'TPR', 'AUC', 'AUPR', 'F05', 'F1', 'F2']
        self.metric_name = ['Precision', 'Recall', 'AUC', 'AUPR', 'F0.5', 'F1', 'F2', 'ACC']
    
    def __call__(self):
        cm = ConfusionMatrix(self.y_true, self.y_pred)
        for j in range(len(self.metrics)):
            for i in range(self.class_num):
                self.result[i][self.metric_name[j]] = str2float(eval('cm.{}'.format(self.metrics[j]))[i])

        return self.result, cm

def classification_metric(y_true, y_pred, class_num, label, save_path):
    metric = TestMetric(y_true, y_pred, class_num)
    class_report, cm = metric()
    ...
    cm_table.title = 'Confusion Matrix'
    cm_table.field_names = [''] + label
    for i, row in enumerate(cm_array):
        cm_table.add_row([label[i]] + list(row))
    if class_num <= 20:
        print(cm_table)
    else:
        print(f"(Matrix too large to display, saved to {save_path}/confusion_matrix.csv)")
    
    if class_num <= 50:
        plot_confusion_matrix(cm.to_array(), label, save_path)
    save_confusion_matrix(cm.to_array(normalized=True), label, save_path)

    table1_cols_name = ['class'] + metric.metric_name
    table1 = PrettyTable()
...
def autobatch(model, imgsz=640, fraction=0.8, batch_size=16):
    ...
    if b < 1:  # zero or negative batch size
        raise RuntimeError(f'{prefix}WARNING: ⚠️ CUDA anomaly detected, recommend restart environment and retry command.')
...
class MetricDataset(torch.utils.data.dataset.Dataset):
...
def freeze_backbone(model, freeze=True):
    """
    Freeze or unfreeze backbone layers, only keep classifier trainable
    Args:
        model: The model
        freeze: True to freeze, False to unfreeze
    """
    # Get classifier layer names (different models have different classifier names)
    ...
    for name, param in model.named_parameters():
        # Check if it's a classifier layer
        is_classifier = any(clf_name in name for clf_name in classifier_names)
    ...
    print(f'Backbone {status}: {frozen_count} layers frozen, {trainable_count} layers trainable')
...
class Model_Inference:
    ...
    def forward_features(self, inputs):
        try:
            ...
            raise TypeError('this model is not a torch model.')
